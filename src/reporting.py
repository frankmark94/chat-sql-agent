...
def create_report_from_messages(
    messages: List[Dict[str, str]], filename: str = "chat_report.pdf"
) -> str:
    """
    ...
    with PdfPages(filename) as pdf:
        ...
        for i in range(0, len(messages), messages_per_page):
            ...
            ax.text(
                0.1,
                0.95,
                f"Chat Messages (Page {i//messages_per_page + 1})",
                transform=ax.transAxes,
                fontsize=16,
                weight="bold",
            )
